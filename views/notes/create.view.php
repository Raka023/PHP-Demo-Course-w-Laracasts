...
<main class="bg-black min-h-screen">
    <div class="mx-auto max-w-7xl px-4 py-6 sm:px-6 lg:px-8">
        <div class="rounded-lg border border-zinc-800 bg-zinc-950 p-8 relative">
            <h2 class="text-2xl font-bold text-zinc-100 mb-6">Create a Note</h2>
            <form method="POST" class="space-y-6" novalidate>
                <div>
                    <label for="body" class="block text-md font-medium text-zinc-200 mb-2">Note</label>
                    <textarea id="body" name="body" required placeholder="Here's an idea for a note..." rows="6" class="block w-full rounded-md border border-zinc-800 bg-zinc-900/50 text-zinc-100 p-4 focus:border-zinc-500 focus:ring-2 focus:ring-zinc-500 resize-none"><?= htmlspecialchars($_POST['body'] ?? '') ?></textarea>
                </div>
